...
class Recommender:
    ...
    def fit(self, train, test, verbose=False):
        ...
        for epoch_num in range(self._n_iter):
            ...
            # Concatenate targets and negative samples
            ...
            end_time = time()
            print(f"Epoch {epoch_num + 1}/{self._n_iter} [{end_time - start_time:.2f}s]\tloss={loss.item():.4f}")
...
def train_model(model, train_data, test_data, config, pretrained_model_path=None, is_pretrain=True):
    ...
    model.save_model('edx_pretrained_model.pth' if is_pretrain else 'kaggle_finetuned_model.pth')
...
if __name__ == '__main__':
    ...
    train_model(kaggle_model, kaggle_train, kaggle_test, config, pretrained_model_path='edx_pretrained_model.pth', is_pretrain=False)
